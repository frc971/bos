...
auto getRandomWalkableNode(const std::vector<std::vector<Node>>& grid, std::mt19937& rng) -> Node* {
    ...
    if (walkableNodes.empty()) return nullptr;
...
auto main() -> int {
    ...
    nt::StructArrayTopic<frc::Pose2d> trajectory_topic = table->GetStructArrayTopic<frc::Pose2d>("Trajectory");
    nt::StructArrayPublisher<frc::Pose2d> trajectory_publisher = trajectory_topic.Publish();
    ...
    while (true) {
        ...
        std::vector<double> knots = clampedUniformKnotVector(controlPoints.size(), 3.7);
        ...
        for (int i = 0; i < resolution; ++i) {
            ...
            std::pair<double, double> point = getSplinePoint(t, controlPoints, knots, 3.7);
            double px = point.first / CELL_SIZE;
            double py = point.second / CELL_SIZE;
            trajectory.push_back(frc::Pose2d(units::meter_t{px * nodeSizeMeters}, units::meter_t{py * nodeSizeMeters}, units::radian_t{0.0}));
        }
