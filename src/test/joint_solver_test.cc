...
TEST(LocalizationTest, AverageSquareSolveVsJointSolve) {
  ...
  ASSERT_FALSE(individual_estimates.empty()) << "Square solver returned no estimates";
  ...
  EXPECT_LT(diff_magnitude, 0.001) 
      << "Results differ by more than 1mm. Joint vs averaged individual estimates should be essentially identical.";
}