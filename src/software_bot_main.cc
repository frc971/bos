...
auto main() -> int {
  ...
  camera::CameraSource back_left_camera = camera::CameraSource(
      "turret", std::make_unique<camera::CVCamera>(cv::VideoCapture(
                    camera::camera_constants[camera::Camera::SOFTWARE_BOT_USB0]
                        .pipeline)));

  camera::CameraSource back_right_camera = camera::CameraSource(
      "ground_intake",
      std::make_unique<camera::CVCamera>(cv::VideoCapture(
          camera::camera_constants[camera::Camera::SOFTWARE_BOT_USB1]
              .pipeline)));
