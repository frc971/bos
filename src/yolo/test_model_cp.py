...
def evaluate():
    ...
    tp_examples = []  # Images with true positives
    fp_examples = []  # Images with false positives
    fn_examples = []  # Images with false negatives
    tn_examples = []  # Images with true negatives (no predictions, no GT)
    ...
    for idx, (preds, gts) in enumerate(zip(all_preds, all_gts)):
        ...
        has_fp = len(fp_preds) > 0
        has_fn = len(gts) - len(matched_gt) > 0
        has_tn = len(preds) == 0 and len(gts) == 0
        ...
        # Collect examples
        if has_tp and len(tp_examples) < EXAMPLES_PER_TYPE:
            tp_examples.append(idx)
        if has_fp and len(fp_examples) < EXAMPLES_PER_TYPE:
            fp_examples.append(idx)
        if has_fn and len(fn_examples) < EXAMPLES_PER_TYPE:
            fn_examples.append(idx)
        if has_tn and len(tn_examples) < EXAMPLES_PER_TYPE:
            tn_examples.append(idx)
        
        # Debug first few images
        if idx < 3:
            print(f"Image {idx}: {len(preds)} preds, {len(gts)} GTs, {len(matched_gt)} matched")
