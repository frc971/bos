...
static inline std::string IMX296Pipeline(int sensor_id, int framerate) {
  std::string pipeline =
      ...
      "video/x-raw(memory:NVMM), width=1456, height=1088, framerate=" +
...
inline CameraInfo IMX296Template(int camera_id, int framerate) {
  camera::CameraInfo camera_info = {
      .pipeline = camera::IMX296Pipeline(camera_id, framerate),
      .name = "camera_" + std::to_string(camera_id),
      .intrinsics_path =
          "constants/camera" + std::to_string(camera_id) + "_intrinsics.json",
      .extrinsics_path =
          "constants/camera" + std::to_string(camera_id) + "_extrinsics.json",
      .id = camera_id};
  return camera_info;
}
