...
MultiCameraSource::MultiCameraSource(
  ...
  for (size_t i = 0; i < cameras_.size(); i++) {
    camera_threads_.emplace_back(
        [this, i, log_path](const std::stop_token& stop_token) -> void {
          const std::string camera_log_dest = fmt::format(
              "{}/{}", log_path, cameras_[i]->GetCameraConstant().name);
