...
class SimpleKalman {
 public:
  ...
  void Update(double position_update, double time, double variance);
  ...
  double position_variance() const { return kalman_filter_.P(0, 0); }
  double velocity_variance() const { return kalman_filter_.P(1, 1); }

  void set_position(double position) { kalman_filter_.state()[0] = position; }
  void set_velocity(double velocity) { kalman_filter_.state()[1] = velocity; }

  void set_measurment_variance(double variance) {
    kalman_filter_.R << variance;
  }
